<?php
/**
 * Plugin Name:       WDT Breadcrumbs
 * Plugin URI:        https://webdevelopmenttech.com/
 * Description:       use shortcode `[wdt_site_breadcrumb]` anywhere you want to show breadcrumbs options for shortcode are seperator=">" and title="Custom title" parent_page="true". Note title will only work with parent_page true.
 * Version:           1.0.0
 * Requires at least: 5.2
 * Tested up to: 	  5.8
 * Requires PHP:      7.2
 * Author:            WDT, Nilesh Kumar Chouhan 
 * Author URI:        https://webdevelopmenttech.com/
 * License:           GPL v2 or later
 * License URI:       https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain:       wdt-breadcrumbs
 * Domain Path:       /languages
 */

// Add Shortcode
add_shortcode( 'wdt_site_breadcrumb', 'wdt_site_breadcrumb_func' );
function wdt_site_breadcrumb_func( $atts ) {
	$atts = shortcode_atts(
		array(
			'seperator' => '>',
			'title' => '',
	...
	if( $atts['parent_page']){
		return '<div class="breadcrumb">' . wdt_full_breadcrumb($atts['seperator']) . '</div>';
	}else{
		return '<div class="breadcrumb">' . wdt_get_breadcrumb( $atts['seperator'], $atts['title'] ) . '</div>';
	}
}

function wdt_get_breadcrumb( $seperator = '&#62;', $title = '' ) {
    ...
    if (is_category() || is_single()) {
        ...
        the_category(' &bull; ');
            if (is_single()) {
                echo " &nbsp;&nbsp;".$seperator."&nbsp;&nbsp; ";
                echo !empty($title) ? $title : the_title();
            }
    } elseif (is_page()) {
        echo "&nbsp;&nbsp;".$seperator."&nbsp;&nbsp;";
        echo !empty($title) ? $title : the_title();
...
function wdt_full_breadcrumb($seperator) {
	// Define
	global $post;
	...
	<style>
		...
		ul#ah-breadcrumb li.item {
		    width: auto;
		    display: inline-block;
		    float: left;
		}
		ul#ah-breadcrumb li.seperator {
		    float: left;
		    display: inline-block;
		    padding: 0 5px;
		}
	...
	// Creating home link
	echo '<li class="item item-home"><a href="'. get_home_url() .'">'. esc_html( $defaults['home_title'] ) .'</a></li>' . $sep;
